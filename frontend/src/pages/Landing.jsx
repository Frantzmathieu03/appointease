      ...
      <div className="min-h-screen bg-slate-50 font-sans">
        ...
        <nav className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setShowLogin(true)} className="text-slate-600 hover:text-indigo-600 font-medium text-sm transition">Sign in</button>
            <button onClick={() => setShowLogin(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-5 py-2 rounded-lg transition">Get started free</button>
          </div>
        </nav>
        ...
        <section className="max-w-6xl mx-auto px-6 py-20 text-center">
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            <button onClick={() => setShowLogin(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-8 py-3.5 rounded-xl text-lg transition">Start free trial</button>
            <a href="#video" className="text-slate-600 hover:text-indigo-600 font-semibold px-8 py-3.5 rounded-xl text-lg border border-slate-300 hover:border-indigo-400 transition">Watch how it works</a>
          </div>
        ...
        <section id="video" className="bg-white py-20">
          <div className="max-w-4xl mx-auto px-6 text-center">
            ...
            <p className="text-slate-500 mb-10">Watch how easy it is to set up and start sending reminders in under 2 minutes</p>
            <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm">
              <iframe
                width="100%"
                height="480"
                src="https://www.youtube.com/embed/ysz5S6PUM-U"
                title="How AppointEase works"
                frameBorder="0"
        ...
        <section className="max-w-6xl mx-auto px-6 py-20">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: '📱', color: 'bg-emerald-50', title: 'SMS + Email reminders', desc: 'Automatic reminders sent at exactly the right time. You choose when — 24 hours, 2 hours, or any time you want.' },
              { icon: '📅', color: 'bg-indigo-50', title: 'Easy rescheduling', desc: 'Clients can cancel or reschedule right from their phone. No calls needed. Your calendar updates instantly.' },
              { icon: '⚙️', color: 'bg-amber-50', title: 'Fully customizable', desc: 'Set your own reminder schedule and message. A law firm and a barbershop have different needs — your app adapts to both.' },
              { icon: '🚫', color: 'bg-rose-50', title: 'No double booking', desc: 'Smart scheduling prevents two clients from booking the same slot. Your calendar stays clean automatically.' },
              { icon: '🔔', color: 'bg-blue-50', title: 'Instant notifications', desc: 'Get notified the moment a client books, cancels, or reschedules. Always know what is happening in your business.' },
              { icon: '💰', color: 'bg-purple-50', title: 'More revenue', desc: 'Every no-show costs you money. Our customers report up to 80% fewer no-shows after switching to AppointEase.' },
            ].map((f, i) => (
        ...
        <section className="max-w-6xl mx-auto px-6 py-20">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            <div className="bg-white rounded-2xl border border-slate-200 p-8">
              ...
              <div className="text-4xl font-extrabold text-slate-800 my-4">$29<span className="text-lg text-slate-400 font-normal">/mo</span></div>
              ...
              <button onClick={() => setShowLogin(true)} className="w-full mt-8 border border-indigo-600 text-indigo-600 hover:bg-indigo-50 font-semibold py-2.5 rounded-xl transition">Get started</button>
            </div>
            <div className="bg-indigo-600 rounded-2xl p-8 relative">
              ...
              <button onClick={() => setShowLogin(true)} className="w-full mt-8 bg-white text-indigo-600 hover:bg-indigo-50 font-semibold py-2.5 rounded-xl transition">Get started</button>
            </div>
            <div className="bg-white rounded-2xl border border-slate-200 p-8">
        ...
        <section className="bg-indigo-600 py-20">
          <div className="max-w-3xl mx-auto px-6 text-center">
            ...
            <button onClick={() => setShowLogin(true)} className="bg-white text-indigo-600 hover:bg-indigo-50 font-bold px-10 py-4 rounded-xl text-lg transition">Start your free trial today</button>
          </div>
        </section>
